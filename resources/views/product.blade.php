...
@section('content')
  <div class="container">
    <ul class="breadcrumb" itemscope itemtype="http://schema.org/BreadcrumbList">
      <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a class="breadcrumb__link" href="/" itemprop="item"><span itemprop="name">Православный интернет-магазин «ВЕЧЕРИЯ»</span>
          <meta itemprop="position" content="1"/>
        </a></li>
      <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a class="breadcrumb__link" href="/catalog/{{$product->category_slug}}" itemprop="item"><span itemprop="name">{{$product->category}}</span>
    ...
    <div class="product" id="{{$product->product_id}}" itemscope itemtype="http://schema.org/Product">
      <div class="product__wrapper">
        <div class="product__col product__col--left">
          ...
          <div class="product__article" itemprop="model">{{$product->article}}</div>
          ...
          <div class="product__image">
            <div class="product-image">
              <div class="product-image__showcase">
                <picture>
                  <source srcset="/images/items/{{$product->image_path[0]}}.webp" data-src="/images/items/{{$product->image_path[0]}}" type="image/webp">
                  <img src="/images/items/{{$product->image_path[0]}}.jpg" data-src="/images/items/{{$product->image_path[0]}}.jpg" alt="{{$product->name}}" title="{{$product->name}}" itemprop="image">
                </picture>
              </div>
              <ul class="product-image__preview">@foreach ($product->image_path as $image)
                  <li>
                    <picture>
                      <source srcset="/images/items/{{$image}}.webp" data-src="/images/items/{{$image}}.webp" type="image/webp">
                      <img src="/images/items/{{$image}}.jpg" data-src="/images/items/{{$image}}.jpg" alt="{{$product->name}}" title="{{$product->name}}">
                    </picture>
                  </li>@endforeach
              </ul>
            </div>
        ...
        <div class="product__col" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
          ...
          <div class="product__row">
            <ul class="product__params">@if ($product->manufacturer)
              ...
              @if ($product->material)
                <li><span>Состав:</span><span>{{$product->material}}</span></li>@endif
              @if ($product->technic)
                <li><span>Техника:</span><span>{{$product->technic}}</span></li>@endif
              @if ($product->dimension)
                <li><span>Количество:</span><span>{{$product->dimension}}</span></li>@endif
              <li><span>Наличие:</span><span class="green" itemprop="availability" content="http://schema.org/InStock">{{$product->available}}</span></li>
          ...
          <div class="product-desc__body">@if ($product->description)
              ...
              <div class="product-desc__screen" data-id="3">
                <div class="text text--product">
                  ...
                  <p>
                    При общей сумме покупки <span class="red">менее 4&nbsp;000&nbsp;руб.</span>, стоимость доставки <strong>в пределах МКАД</strong> составляет – <strong>300&nbsp;руб.</strong><br/>При заказе на сумму <span class="red">4&nbsp;000&nbsp;руб. и более</span>,
                    доставка <strong>в пределах МКАД</strong> бесплатная.</p>
                  <p>
                    Доставка осуществляется на следующий день <span class="red">с&nbsp;10.00 до&nbsp;20.00</span> со дня оформления заказа.<br/>Доставка <strong>«день в день»</strong> возможна, если заказ поступил <span
                      class="red">до&nbsp;12.00</span>.</p>
                  <div class="text__title">
                    <h2>В Ближайшее Подмосковье</h2>
                  </div>
                  <p>
                    При общей сумме покупки <span class="red">менее 10&nbsp;000&nbsp;руб.</span>, стоимость доставки оплачивается по тарифам «Почты России» или по тарифам курьерской службы.</br>При заказе на сумму <span class="red">10&nbsp;000&nbsp;руб. и более</span>,
                    доставка «Почтой России» или курьерской службой бесплатная.</p>
                  <p>Стоимость и срок доставки в Ваш регион уточняйте у нашего менеджера по телефону!</p>
                  <div class="text__title">
                    <h2>По России</h2>
                  </div>
                  <p>
                    Посылки отправляются <strong>«Почтой России»</strong> <a target="_blank" href="https://www.pochta.ru/support/post-rules/cash-on-delivery">наложенным платежом</a>:</br> - заказ прибудет в отделение «Почты России» по удобному для
                    Вас адресу.
                  ...
                  <p>
                    Учитывая тот факт, что изделия мастерских - это продукт ручной работы, понравившиеся Вам изделия могут отсутствовать в наличии и изготавливаются на заказ. Изготовление православного изделия на заказ осуществляется <span
                      class="red">после 100% авансового платежа (предоплаты)</span>.</p>
